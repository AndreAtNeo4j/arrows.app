...
import { NEW_GRAPH_TEMPLATE } from '@arrows-code/messages';
...
// Shared starter graph (messages/new-graph.json) so both hosts seed the same content.
...
export function makeNewFromExample(context: vscode.ExtensionContext) {
  return async (arg?: unknown): Promise<void> => {
    ...
    if (!source.fsPath.startsWith(examplesDir(context).fsPath + sep)) {

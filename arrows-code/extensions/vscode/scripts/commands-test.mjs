...
exports.run = async function () {
  const ext = vscode.extensions.getExtension('neo4j-labs.arrows-code-vscode');
  ...
  {
    const doc = await openFixture();
    const restoreQp = stub(vscode.window, 'showQuickPick', async (items) => items.find((i) => i.clause === 'CREATE'));
    try { await vscode.commands.executeCommand('arrows.copyCypher', doc.uri); } finally { restoreQp(); }
  ...
  console.log('\\n▸ arrows.exportCypher');
  {
    ...
    const restoreSave = stub(vscode.window, 'showSaveDialog', async () => target);
    const restoreQp = stub(vscode.window, 'showQuickPick', async (items) => items.find((i) => i.clause === 'CREATE'));
    try { await vscode.commands.executeCommand('arrows.exportCypher', doc.uri); } finally { restoreSave(); restoreQp(); }

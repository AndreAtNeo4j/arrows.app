...
import { LAYOUTS, findLayout, type LayoutId } from '@neo4j-arrows/graph-logic';
    ...
    if (showSpinner) {
      laidOut = await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          return (await chosen.run(graph as unknown as Parameters<typeof chosen.run>[0], reportProgress)) as unknown as typeof graph;
        }
      );
    } else {
      laidOut = (await chosen.run(graph as unknown as Parameters<typeof chosen.run>[0])) as unknown as typeof graph;
    }

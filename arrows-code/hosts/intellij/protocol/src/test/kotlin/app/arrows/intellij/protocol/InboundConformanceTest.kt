 ...
 * (host-protocol/fixtures/inbound-messages.json). If the two parsers drift, this fails.
 */
class InboundConformanceTest {
    ...
    private fun normalize(m: InboundMessage?): Map<String, Any?>? = when (m) {
        ...
        is InboundMessage.GraphChanged -> mapOf(
            ...
            "nodeCount" to m.graph.nodes.size.toLong(),
            "relCount" to m.graph.relationships.size.toLong(),
